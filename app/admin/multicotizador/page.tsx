...
// Multicotizador — entorno de multicotización inteligente. Todavía no está
// habilitado para los brokers: la página muestra un cartel informativo y un
// botón de WhatsApp para solicitar el alta. El mensaje incluye el nombre del
// negocio registrado (localStorage "aseguradora").
...
function Multicotizador() {
  ...
  useEffect(() => {
    try {
      ...
      if (a) setNegocio(JSON.parse(a)?.nombre || "")
    } catch {}
  }, [])

  const mensaje = `Hola${negocio ? `, soy ${negocio}` : ""}, quiero solicitar información sobre el Multicotizador inteligente de SegurOS.`
